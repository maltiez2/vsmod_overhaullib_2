    ...
    where TKey : notnull
{
    ...
    public void Clean()
    {
        ...
        try
        {
            Log.Verbose(_api, this, $"({_loggedCacheName}) Starting clean up. Current world time: {TimeSpan.FromMilliseconds(currentTime)}\nSize: {_mapping.Count}\n'Get' count: {_getCountBetweenCleanUps}\n'Add' count: {_addCountBetweenCleanUps}");
            ...
            Log.Verbose(_api, this, $"({_loggedCacheName}) Cleaned up '{keysToRemove.Count}' keys for '{entities.Count}' values.");
        }
        catch (Exception exception)
        {
            Log.Error(_api, this, $"({_loggedCacheName}) Error on cache cleanup:\n{exception}");
        }
